...
class Adafruit_VC0706 {
 public:
  ...
  Adafruit_VC0706(); // Constructor when using HardwareSerial
  //boolean begin(uint16_t baud = 38400);
  boolean begin(long baud);
  boolean end();
  boolean changeBaudrate(long baud);
  boolean reset(void);
  boolean TVon(void);
  boolean TVoff(void);
