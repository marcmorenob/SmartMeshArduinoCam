...
// Constructor when using HardwareSerial
Adafruit_VC0706::Adafruit_VC0706() {
  common_init(); // Set everything to common state, then...
}

boolean Adafruit_VC0706::begin(long baud) {
  Serial2.begin(baud);
  return reset();
}
...
//Change to 115200
boolean Adafruit_VC0706::changeBaudrate(long baud) {
  uint8_t arg[4] = {0x03,0x01};

  switch(baud){
    case 9600: arg[2]=0xAE;
           arg[3]=0xC8;
           break;
    case 19200:arg[2]=0x56;
           arg[3]=0xE4;
           break;
    case 38400:arg[2]=0x2A;
           arg[3]=0xF2;
           break;
    case 57600:arg[2]=0x1C;
           arg[3]=0x1C;
           break;
    case 115200:arg[2]=0x0D;
            arg[3]=0xA6;
           break;
    default:   return false;
        
   }
  return runCommand(VC0706_SET_PORT, arg, 4, 6);
}
...
boolean Adafruit_VC0706::setImageSize(uint8_t x) {
  ...
  return runCommand(VC0706_WRITE_DATA, args, sizeof(args), 6);
}
...
/*
 * c = 0x0 automatically step black-white and colour.
 * c = 0x1 manually step color, select colour.
 * c = 0x2 manually step color, select black-white
 */
boolean Adafruit_VC0706::setColorCtl(uint8_t c) {
  uint8_t args[] = {0x2, 0x1, c};
  return runCommand(VC0706_COLOR_CTRL, args, sizeof(args), 6);
}

uint8_t Adafruit_VC0706::getColorStatus(void) {
  uint8_t args[] = {0x0};
  runCommand(VC0706_COLOR_STATUS, args, sizeof(args), 6);
  //printBuff();
  return camerabuff[6];
}
...
void Adafruit_VC0706::sendCommand(uint8_t cmd, uint8_t args[] = 0, uint8_t argn = 0) {
  if(swSerial) {
    ...
    Serial2.write((byte)serialNum);
    Serial2.write((byte)cmd);

    for (uint8_t i=0; i<argn; i++) {
      Serial2.write((byte)args[i]);
      //Serial.print(" 0x");
      //Serial.print(args[i], HEX);
    }
#else
    Serial2.print(0x56, BYTE);
    Serial2.print(serialNum, BYTE);
    Serial2.print(cmd, BYTE);

    for (uint8_t i=0; i<argn; i++) {
      Serial2.print(args[i], BYTE);
      //Serial.print(" 0x");
      //Serial.print(args[i], HEX);
    }
#endif
...
uint8_t Adafruit_VC0706::readResponse(uint8_t numbytes, uint8_t timeout) {
  ...
  while ((timeout != counter) && (bufferLen != numbytes)){
    avail = swSerial ? swSerial->available() : Serial2.available();
    if (avail <= 0) {
      delay(1);
      counter++;
      continue;
